...
TaskQueue<T, MAX_PRIORITY>::TaskQueue(int64_t c) {
  assert(c && (!(c & (c-1))));
  unroll<unsigned, 0, MAX_PRIORITY, unsigned, 1>([&](unsigned p){
...
size_t TaskQueue<T, MAX_PRIORITY>::size() const noexcept {
  size_t s;
  unroll<unsigned, 0, MAX_PRIORITY, unsigned, 1>([&](unsigned i) { s = i ? size(i) + s : size(i); });
  return s;
}
...
int64_t TaskQueue<T, MAX_PRIORITY>::capacity() const noexcept {
  size_t s;
  unroll<unsigned, 0, MAX_PRIORITY, unsigned, 1>([&](unsigned i) { 
